...
@Component({
  ...
  templateUrl: './conges-valides.component.html',
  styleUrls: ['./conges-valides.component.css']
})
export class CongesValidesComponent implements OnInit {
  ...
  checkWeekendsWithYear(day: Date, year) {
    ...
    return d === 'dim.' || d === 'sam.' || CongesValidesComponent.joursFeries(year).includes(moment(day).format('DD/MM/YYYY').toString());
  }
